...
export class EventsService {
    ...
    private async findSimilarEvents(event: { id: string; categoryId: string; location: string }) {
        const SimilarEventsLimit = 5;
        const ScoreSameCategory = 2;
        const ScoreSameCity = 1;
        ...
        const candidates = await db
            ...
            .where(
                and(
                    ...
                    or(eq(eventsTable.categoryId, event.categoryId), like(eventsTable.location, `%${city}%`))
                )
            );
        ...
        return candidates
            .map((candidate) => {
                const score =
                    (candidate.categoryId === event.categoryId ? ScoreSameCategory : 0) +
                    (extractCity(candidate.location) === city ? ScoreSameCity : 0);

                return { ...candidate, score };
            })
            .sort((a, b) => b.score - a.score)
            .slice(0, SimilarEventsLimit)
